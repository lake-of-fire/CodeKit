...
public class Room: Object, DBSyncableObject, UnownedSyncableObject {
...
//    func submit(message: ConversationMessage) async {
//        let context = context(endingWithMessage: message)
//        let openAI = ProxiedOpenAI()
//        let chatMessages = context.compactMap { (message: ConversationMessage) -> ChatMessage? in
//            let role: ChatRole
...
//        }
//        let chatParameters = ChatParameters(model: "gpt-3.5-turbo", messages: chatMessages)
...
//        } catch {
//            message.didFailToDeliver = true
//            return
//        }
//        guard let choice = choice, let realm = realm?.thaw() else {
//            message.didFailToDeliver = true
//            return
//        }
//        try! realm.write {
//            realm.create(ConversationMessage.self, value: [
//                "conversation": self,
...
//        message.didFailToDeliver = false
//    }
    
//    func context(endingWithMessage message: ConversationMessage) -> [ConversationMessage] {
//        return messages.where { $0.createdAt <= message.createdAt && $0.isDeleted == false }.sorted(by: \.createdAt).map { $0 }
//    }
}

public class Persona: Object, DBSyncableObject, UnownedSyncableObject {
    @Persisted(primaryKey: true) public var id = UUID()
...
//    public static func getOrCreateBotHackaround() -> Persona {
//        let realm = try! Realm()
//        if let user = realm.objects(Persona.self).where({ $0.personaType == .bot && $0.name == "ChatBOT" && !$0.isDeleted }).first {
//            return user
//        }
//        let user = Persona()
//        user.name = "ChatBOT"
//        user.personaType = .bot
//        safeWrite { realm in
//            realm.add(user)
//        }
//        return user
...
public class Event: Object, DBSyncableObject, UnownedSyncableObject {
    @Persisted(primaryKey: true) public var id = UUID()
...
public class Link: Object, DBSyncableObject, UnownedSyncableObject {
    @Persisted(primaryKey: true) public var id = UUID()
